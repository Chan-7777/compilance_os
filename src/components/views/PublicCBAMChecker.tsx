...
export function PublicCBAMChecker() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: colors.background, fontFamily: 'inherit' }}>
      {/* Header */}
      <div style={{ backgroundColor: colors.sidebar, padding: `${spacing.lg} ${spacing.xl}`, display: 'flex', alignItems: 'center', gap: spacing.md }}>
        <div style={{ fontSize: '0.7rem', fontWeight: 800, letterSpacing: 3, textTransform: 'uppercase', color: colors.sidebarText }}>ComplianceOS</div>
      </div>

      <div style={{ maxWidth: 640, margin: '0 auto', padding: `${spacing.xl} ${spacing.lg}` }}>
        {/* Hero */}
        <div style={{ marginBottom: spacing.xl }}>
          <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, color: colors.accent, marginBottom: spacing.xs }}>
        ...
        {results && (
          <div style={{ marginBottom: spacing.xl }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm, marginBottom: spacing.lg }}>
              {results.map(r => (
                <div key={r.code} style={{
                  ...
                  border: `1px solid ${r.inScope ? colors.status.error + '66' : colors.status.success + '66'}`,
                  backgroundColor: r.inScope ? colors.surfaces.dangerBg : colors.surfaces.successBg,
                  ...
                  <div>
                    ...
                    {r.inScope && r.sector && (
                      <div style={{ fontSize: '0.75rem', color: colors.surfaces.dangerText, marginTop: 2 }}>
                        Sector: {r.sector}
                      </div>
                    )}
                    {!r.inScope && (
                      <div style={{ fontSize: '0.75rem', color: colors.status.success, marginTop: 2 }}>
                  ...
                  <div style={{
                    ...
                    backgroundColor: r.inScope ? colors.status.error : colors.status.success, color: colors.white,
                    flexShrink: 0,
                  }}>
            ...
            {anyInScope && (
              <div style={{
                padding: spacing.md, backgroundColor: colors.surfaces.amberBg, borderRadius: borderRadius.md,
                border: `1px solid ${colors.status.pending}44`, fontSize: '0.8rem', color: colors.surfaces.amberText, marginBottom: spacing.lg,
              }}>
            ...
            {/* Timeline */}
            <div style={{
              backgroundColor: colors.white, borderRadius: borderRadius.lg, border: `1px solid ${colors.border}`,
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
                {CBAM_TIMELINE.map(t => (
                  <div key={t.date} style={{ display: 'flex', gap: spacing.md, alignItems: 'flex-start' }}>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', color: colors.accent, fontWeight: 700, minWidth: 140, flexShrink: 0 }}>
                      {t.date}
                    </div>
                    <div>
            ...
            <div style={{
              ...
              border: `2px solid ${colors.accent}44`, padding: spacing.lg, textAlign: 'center',
            }}>
              <div style={{ fontSize: '1rem', fontWeight: 700, color: colors.primary, marginBottom: spacing.xs }}>
                Get your full RoDTEP entitlement estimate
              </div>
              <div style={{ fontSize: '0.8rem', color: colors.accent, marginBottom: spacing.md }}>
                Indian exporters leave ₹ crores in RoDTEP rebates unclaimed every year. Sign up free to see yours.
              </div>
              <a
                href="/"
                style={{
                  display: 'inline-block', padding: '10px 28px', backgroundColor: colors.accent, color: colors.white,
