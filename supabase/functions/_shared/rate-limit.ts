...
export async function checkRateLimit(userId, fnName) {
  const limit = DAILY_LIMITS[fnName] ?? 100;
  const serviceClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
  const today = new Date().toISOString().slice(0, 10);
  // Upsert: increment count or create row
  const { data, error } = await serviceClient.from('rate_limits').upsert({
    user_id: userId,
    fn_name: fnName,
    window_date: today,
    count: 1,
  ...
  if (error) {
    ...
    const { data: existing } = await serviceClient.from('rate_limits').select('count').eq('user_id', userId).eq('fn_name', fnName).eq('window_date', today).maybeSingle();
    const currentCount = (existing?.count ?? 0) + 1;
    await serviceClient.from('rate_limits').update({
      count: currentCount,
      updated_at: new Date().toISOString()
    }).eq('user_id', userId).eq('fn_name', fnName).eq('window_date', today);
    return {
